...
class ChangeCompletedEntriesUseCase(private val repository: IProfileRepository) : UseCase<CounterEntriesParams, None>() {

    override suspend fun run(params: CounterEntriesParams): Either<Failure, None> {
        return repository.changeCompletedEntries(params.profileId)
    }
}